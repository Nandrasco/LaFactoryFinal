import { Component, OnInit, OnDestroy } from '@angular/core';
import { ActivatedRoute, Router } from '@angular/router';

import { NgbActiveModal, NgbModal, NgbModalRef } from '@ng-bootstrap/ng-bootstrap';
import { JhiEventManager } from 'ng-jhipster';

import { IOrdinateur } from 'app/shared/model/ordinateur.model';
import { OrdinateurService } from './ordinateur.service';

@Component({
    selector: 'jhi-ordinateur-delete-dialog',
    templateUrl: './ordinateur-delete-dialog.component.html'
})
export class OrdinateurDeleteDialogComponent {
    ordinateur: IOrdinateur;

    constructor(private ordinateurService: OrdinateurService, public activeModal: NgbActiveModal, private eventManager: JhiEventManager) {}

    clear() {
        this.activeModal.dismiss('cancel');
    }

    confirmDelete(id: number) {
        this.ordinateurService.delete(id).subscribe(response => {
            this.eventManager.broadcast({
                name: 'ordinateurListModification',
                content: 'Deleted an ordinateur'
            });
            this.activeModal.dismiss(true);
        });
    }
}

@Component({
    selector: 'jhi-ordinateur-delete-popup',
    template: ''
})
export class OrdinateurDeletePopupComponent implements OnInit, OnDestroy {
    private ngbModalRef: NgbModalRef;

    constructor(private activatedRoute: ActivatedRoute, private router: Router, private modalService: NgbModal) {}

    ngOnInit() {
        this.activatedRoute.data.subscribe(({ ordinateur }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(OrdinateurDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
                this.ngbModalRef.componentInstance.ordinateur = ordinateur;
                this.ngbModalRef.result.then(
                    result => {
                        this.router.navigate([{ outlets: { popup: null } }], { replaceUrl: true, queryParamsHandling: 'merge' });
                        this.ngbModalRef = null;
                    },
                    reason => {
                        this.router.navigate([{ outlets: { popup: null } }], { replaceUrl: true, queryParamsHandling: 'merge' });
                        this.ngbModalRef = null;
                    }
                );
            }, 0);
        });
    }

    ngOnDestroy() {
        this.ngbModalRef = null;
    }
}
